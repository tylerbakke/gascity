...
## Known flake floor on Mac (cmd/gc)

Several `cmd/gc` tests fail in larger batch runs on local Mac dev
boxes (darwin) due to test-interaction artifacts (TempDir cleanup
races, parallel port allocation, subprocess kill timing). Pass cleanly
when run individually. The same flakes are observable on
`upstream/main` directly — they are not introduced by any local
change.

Stable failures:

- `TestCityRuntimeManualReloadPanicAfterReloadKeepsReloadReplyAndClears`
- `TestCityRuntimeReloadDrainBoundedByTimeout`
- `TestCityRuntimeReloadDrainShortCircuitsOnTickContextCancel`
- `TestCityRuntimeWatchReloadPanicRestoresDirty`
- `TestControllerStateCreatedAgentVisibleAfterStaleRuntimeInterleaving`

Order-dependent failures (intermittent):

- `TestCityRuntimeManualReloadReplyWaitsForTickCompletion`
- `TestOrderRunExecPropagatesManagedDoltLayout`
- `TestRegisterCityWithSupervisorFailsFastWhenSupervisorStopsDuringWait`

Local workaround for parallel-induced flakes only:

```bash
GOFLAGS='-p=1 -parallel=1' go test ./cmd/gc/...
```

This rescues the order-dependent failures but does not fix the 5
stable ones. Tracked in `cc-q3c6c` (command-center).
